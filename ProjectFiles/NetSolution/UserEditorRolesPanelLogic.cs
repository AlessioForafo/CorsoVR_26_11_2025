...
public class UserEditorRolesPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIRoles()
    {
        ...
        foreach (var role in roles.Children)
        {
            ...
            if (editable.Value)
            {
                roleUiObject = InformationModel.MakeObject<Panel>(role.BrowseName, CorsoVR_26_11_2025.ObjectTypes.GroupCheckbox);
            }
            else if (UserHasRole(role.NodeId))
            {
                roleUiObject = InformationModel.MakeObject<Panel>(role.BrowseName, CorsoVR_26_11_2025.ObjectTypes.GroupLabel);
            }
